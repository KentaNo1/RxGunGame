...
CreateThread(function()
    while true do
        ...
        if distance <= 15.0 then
            local gameActive = GlobalState['RX:GunGame:ActiveGame']
            ...
            if gameActive then
                local playersInGame = GlobalState['RX:GunGame:PlayersInGame']
                local currentMap = GlobalState['RX:GunGame:CurrentMap']
                local maximumPlayers = Config.Maps[currentMap].MaximumPlayers
                local roundTimeLeft = GlobalState['RX:GunGame:RoundTimeLeft']
                ...
                if distance <= 1.7 then
                    if not LocalPlayer.state["RX:GunGame:inGame"] then
